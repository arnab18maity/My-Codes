...
*/    
    int minimumOperationsToMakeEqual(int a, int b) {
        if(a == b) return 0;
        if(b > a) return b-a;
        ...
        // dp[b] = 0; // Can ignore it because we are initializing everything with 0
         
        for(int x = b+1; x <= a; x++) {
            int div11 = INT_MAX;
            int div5 = INT_MAX;
        
           if(x % 11 == 0) div11 =  1 + dp[x/11];
           else {
               int prevMultiple = x%11 + 1 + dp[x/11];
               int nextMultiple = (11 - x%11) + 1 + dp[x/11 + 1];
               div11 =  min(prevMultiple,nextMultiple);
           }

           if(x % 5 == 0) div5 =  1 + dp[x/5]; 
           else {
               int prevMultiple = x%5 + 1 + dp[x/5];
               int nextMultiple = (5 - x%5) + 1 + dp[x/5 + 1];
               div5 =  min(prevMultiple,nextMultiple);
           }

           int dec1 = 1 + dp[x-1];

           dp[x] = min({div11,div5,dec1});
        }         
